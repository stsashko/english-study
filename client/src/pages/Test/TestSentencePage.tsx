...
const TestSentencePage: FC = () => {
    ...
    const [currentQuestions, setCurrentQuestions] = useState<{ sentence: String, answer: String } | null>(null);
    ...
    const [sendAnswer] = useMutation(SEND_ANSWER_WITH_QUESTION_SENTENCE_MUTATION, {});
    ...
    const onFinish = async (dataRadio: { rating: String }) => {
        try {
            ...
            if (answer.data.sendAnswerWithQuestionSentence.totalQuestions == answer.data.sendAnswerWithQuestionSentence.totalResponses)
                setFinish(true);

            setSuccessfulAnswer(Boolean(answer.data.sendAnswerWithQuestionSentence.success));
        } catch (e: any) {
    ...
    if (data?.getRandomQuestionSentence?.testIsOver)
        return (
            <Result
                icon={Math.ceil(data.getRandomQuestionSentence.totalSuccesses * 100 / data.getRandomQuestionSentence.totalResponses) >= 80 ?
                    <SmileOutlined style={{color: '#52c41a'}}/> : <FrownOutlined style={{color: '#ff4d4f'}}/>}
                title={(
                    <div>
                        <div>Your rating</div>
                        <Progress percent={100} strokeColor="red"
                                  success={{percent: Math.ceil(data.getRandomQuestionSentence.totalSuccesses * 100 / data.getRandomQuestionSentence.totalResponses)}}
                                  type="circle"
                                  format={() => Math.ceil(data.getRandomQuestionSentence.totalSuccesses * 100 / data.getRandomQuestionSentence.totalResponses)}/>
                    </div>
                )}
                extra={[
                    <Button onClick={() => navigate(`/${TESTS_ROUTE}`)} type="primary" key="got-to-tests">Go to the list
                        of tests</Button>
                ]}
            />
        );
    ...
    return (
        <Content title={TITLE_PAGE} titlePage={TITLE_PAGE}>
            <Title level={3} className={s.title}>Translate the sentence correctly:</Title>
            {pagination.length > 0 && (
                <Space style={{
                    backgroundColor: '#fff',
                    padding: '15px 10px',
                ...
                {currentQuestions && (
                    <Card title={(
                        <React.Fragment>
                            ...
                            {showTranslation && <div style={{color: '#1890ff'}}>{currentQuestions.answer}</div>}
                        </React.Fragment>
                            ...
                            <Radio.Group onChange={onChange} className={s.radioGroup} value={value}>
                                <Row>
                                    <Col span={4} xs={{span: 24}} sm={{span: 12}} md={{span: 12}} lg={{span: 12}}
                                        ...
                                        <Radio value={`${data.getRandomQuestionSentence.questionId}-1`} disabled={disabledRadio && value !== `${data.getRandomQuestionSentence.questionId}-1`} className={s.radio}>I know</Radio>
                                    </Col>
                                    <Col span={4} xs={{span: 24}} sm={{span: 12}} md={{span: 12}} lg={{span: 12}}
                                         style={{marginBottom: '15px', paddingRight: '15px'}}>
                                        <Radio value={`${data.getRandomQuestionSentence.questionId}-0`} disabled={disabledRadio && value !== `${data.getRandomQuestionSentence.questionId}-0`} className={s.radio}>I don't know</Radio>
                                    </Col>
                                </Row>
                            </Radio.Group>
                        </Form.Item>
                        <Row style={{maxWidth: '650px', margin: '0 auto'}}>
                            <Col span={24} style={{padding: '15px'}}>
                                <Button type="primary" onClick={handleNextQuestion} disabled={nextDisabled}
                                        loading={nextLoading}>
