...
class WordQuery {
    words = async (parent, args, context) => {
        ...
        try {
            ...
            const {results} = new Words(await context.prisma.word.findMany({
                where,
                skip: skip,
...
const WordResolver = {
    Word: {
        user(parent, args, context) {
            return context.prisma.word.findUnique({where: {id: parent.id}}).user()
        },
        async bySentence(parent, args, context) {
            try {
                let {results} = new Sentences(await context.prisma.word.findUnique({where: {id: parent.id}}).bySentence());
                return results;
            } catch (e) {
                console.log(e);
                return e;
            }
            return context.prisma.word.findUnique({where: {id: parent.id}}).sentence()
        }
    }
}
